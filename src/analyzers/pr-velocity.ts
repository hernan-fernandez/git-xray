 ...
 */
function findBranchCreationDate(
  startHash: string,
  commitMap: Map<string, CommitRecord>,
  ...
  while (stack.length > 0) {
    ...
    if (mainLineHashes.has(hash)) continue;
 ...
 */
export function analyzePRVelocity(
  ...
  if (mergeRecords.length === 0) {
    return {
      ...
      totalMerges: 0,
      warningMessage:
        'No merge commits found. This repository likely uses a rebase or squash merge workflow. PR velocity metrics are unavailable.',
    };
  }
  ...
  for (const merge of mergeRecords) {
    ...
    if (merge.parentHashes.length < 2) continue;
    const secondParent = merge.parentHashes[1];

    const branchCreationDate = findBranchCreationDate(
      secondParent,
      commitMap,
      mainLineHashes,
    );
