 ...
 */
function computeBusFactor(authorWeights, scope) {
 ...
 */
export function analyzeBusFactor(commits, fileChanges, referenceDate) {
    // --- Overall bus factor: weighted commit counts per author ---
    const overallWeights = new Map();
    for (const commit of commits) {
        const weight = timeDecayWeight(commit.date, referenceDate);
        overallWeights.set(commit.author, (overallWeights.get(commit.author) ?? 0) + weight);
    }
    ...
    const dirAuthorWeights = new Map();
