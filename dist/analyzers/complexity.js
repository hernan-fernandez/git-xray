 ...
 */
export function generateSnapshotDates(from, to, interval) {
    const dates = [];
    if (interval === 'monthly') {
 ...
 */
export function parseLsTreeOutput(output, scope) {
    let totalFiles = 0;
    let totalSize = 0;
 ...
 */
async function findCommitAtDate(date, branch, gitRunner) {
 ...
 */
async function getTreeMetrics(commitHash, gitRunner, scope) {
 ...
 */
export async function analyzeComplexityTrend(config, gitRunner) {
 ...
 */
export function computeChurnRates(snapshots) {
