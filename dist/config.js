        ...
        if (flag.startsWith('--')) {
            ...
            switch (flag) {
                case '--since': {
                    ...
                    if (!value)
                        throw new ConfigError('--since requires a date value');
                    const date = new Date(value);
                    if (isNaN(date.getTime()))
                        throw new ConfigError(`Invalid date for --since: ${value}`);
                    config.since = date;
                    break;
                }
                case '--until': {
                    const value = inlineValue ?? args[++i];
                    if (!value)
                        throw new ConfigError('--until requires a date value');
                    const date = new Date(value);
                    if (isNaN(date.getTime()))
                        throw new ConfigError(`Invalid date for --until: ${value}`);
                    config.until = date;
                    break;
                }
                case '--branch': {
                    const value = inlineValue ?? args[++i];
                    if (!value)
                        throw new ConfigError('--branch requires a branch name');
                    config.branch = value;
                    break;
                }
                case '--scope': {
                    const value = inlineValue ?? args[++i];
                    if (!value)
                        throw new ConfigError('--scope requires a path');
                    config.scope = value;
                    break;
                }
                case '--output': {
                    const value = inlineValue ?? args[++i];
                    if (!value)
                        throw new ConfigError('--output requires a file path');
                    config.output = value;
                    break;
                }
                case '--no-open':
